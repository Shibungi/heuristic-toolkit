...
各ソースの冒頭コメントだけで読み始められるように、次を順番に書いてあります。

1. 問題の要約と、最大化・最小化のどちらか
2. `State` が保持するもの
3. `Action` と1回の遷移
4. ライブラリが担当する採否・選択・巻き戻し
5. コードを追う順序
6. 教材用baselineとして省略している点

まず冒頭コメントを読み、その後に `main` から呼び出し先を追ってください。特に
「目的関数の向き」と「省略している点」を確認せず、そのまま提出解へ流用しないでください。
